...
use legion_project::OpenedFileText;
use legion_protocol::{
...
fn remote_envelope(
    ...
    RemoteTransportEnvelope {
        ...
        event_sequence: legion_protocol::EventSequence(operation_id.0 as u64),
...
fn workspace_vfs_integration_rollback_audit_failure_records_failed_lifecycle() {
    ...
    assert!(matches!(
        ...
        ProposalResponse::Failed {
            reason: legion_protocol::ProposalFailureReason::StorageFailed,
            ..
        }
    ));
    let shell = app
        ...
        vec![
            ...
            ProposalBatchItem {
                ...
                payload: Box::new(ProposalPayload::DeleteFile(
                    legion_protocol::DeleteFileProposal {
                        file: delete_node.identity.clone(),
                    },
                )),
            ...
            ProposalBatchItem {
                ...
                payload: Box::new(ProposalPayload::RenameFile(
                    legion_protocol::RenameFileProposal {
                        file: rename_node.identity.clone(),
...
fn workspace_vfs_integration_batch_contract_records_dependency_blocked_partial_failures() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            (
                "dependent",
                legion_protocol::ProposalPartialFailureDisposition::NotStarted,
            ),
        ]
    );
    assert_eq!(
        contract.items[1].partial_failure_disposition,
        Some(legion_protocol::ProposalPartialFailureDisposition::NotStarted)
    );
    assert_eq!(
        journal
            .items
        ...
        vec![
            ...
            ProposalBatchItem {
                ...
                payload: Box::new(ProposalPayload::TerminalCommand(
                    legion_protocol::TerminalCommandProposal {
                        session_id: None,
...
impl AppWorkspaceCommandPort for MockWorkspacePort {
    ...
    fn tree_snapshot(
        &self,
        workspace_id: WorkspaceId,
    ) -> Result<Vec<FileTreeNode>, legion_app::AppCompositionError> {
        let mut clone = Self {
...
fn mock_descriptor(buffer_id: BufferId, file_id: FileId) -> TextTransactionDescriptor {
    TextTransactionDescriptor {
        ...
        correlation_id: legion_protocol::CorrelationId(99),
...
fn workspace_vfs_integration_command_dispatcher_routes_without_concrete_ports() {
    let active = legion_app::AppCommandRouteContext {
        workspace_id: Some(WorkspaceId(11)),
    ...
    let routed = CommandDispatcher::route_intent(
        ...
        active,
        legion_protocol::CorrelationId(1),
    )
    ...
    let save = CommandDispatcher::route_intent(
        ...
        active,
        legion_protocol::CorrelationId(2),
    )
    ...
    let wrong_buffer = CommandDispatcher::route_intent(
        ...
        active,
        legion_protocol::CorrelationId(3),
    )
...
fn workspace_vfs_integration_phase4_ai_run_is_context_inspectable_and_proposal_only() {
    ...
    assert!(
        outcome
            ...
            .items
            .iter()
            .any(|item| item.kind == legion_protocol::ContextManifestItemKind::ProviderRoute)
    );
    assert!(
        outcome
...
fn workspace_vfs_integration_devcontainer_remote_session_uses_policy_planner() {
    ...
    let descriptor = app
        .connect_devcontainer_workspace_session_from_json(
            ...
            r#"{
                ...
                "remoteUser": "vscode",
                "workspaceFolder": "/workspace/legion"
            }"#,
        )
    ...
    assert_eq!(
        ...
        "legion-remote-devcontainer-agent/1"
    );
    assert!(
        descriptor
