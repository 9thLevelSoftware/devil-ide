...
The implementation must add the corresponding dependency-policy contract note and ownership/contract tests before activating new command variants. No new crate dependency is authorized. The root coordinator owns architecture and final acceptance; Luna workers perform bounded implementation/review under the user's execution policy.
