...
P2.F3.T1 activates `legion-debug` for metadata-only DAP client lifecycle state and protocol projection contracts, plus **WS-A-D B1** live stdio framing and supervised `std::process` spawn of an adapter binary (see `ADR-0044-dap-client-architecture.md`). It may not depend on app, UI, editor, project, terminal, `legion-platform`, storage, security policy, provider, worker, or desktop internals. App-owned trust/capability checks remain outside this crate. P2.F3.T2 keeps that boundary: `legion-debug` never evaluates workspace trust, it only refuses to resolve an adapter binary without an `AdapterResolutionGrant` minted from a granted `debug.adapter.launch` decision (a `legion-protocol` DTO) plus the allowlist that decision was made under. The capability id is therefore duplicated in `legion-security` and `legion-debug` by contract rather than shared through a dependency edge, and an app unit test asserts the two constants agree. Adapter stdout/stderr/source payloads stay metadata-bounded; the app composes `legion-debug` through protocol DTOs.
